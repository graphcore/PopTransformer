...
class BaseRotaryAttention(BaseLayer):
    ...
    def forward_qkv(self, graph, x, position_ids, block_position_ids):
        raise NotImplementedError
    ...
    def fixed_pos_embedding(self, graph, position_id):
        ...
        position_id = ops.cast(
            graph, ops.reshape(graph, position_id, [-1, 1]), self.popart_float_type
        )
    ...
    def rotate_half(self, graph, x):
        ...
        x2 = ops.mul(graph, x2, ops.constant(graph, np.array([-1]).astype(np.float16)))
...
class ShardRotaryAttention(BaseRotaryAttention):
    def collect_bind_layer_weights(self):
        self.c_attn = Linear(
            self.context,
            "query_key_value",
    ...
    def forward_qkv(self, graph, x, layer_past, step, position_ids, block_position_ids):
        qkv = self.c_attn(graph, x, **self.matmul_kwargs)
        qkv = ops.reshape(
            graph,
            qkv,
            [self.batch_size, self.sequence_length, self.num_head, 3 * self.head_size],
        )

        temp_splits = [self.head_size] * 3
        q, k, v = ops.split(graph, qkv, num_outputs=3, axis=3, splits=temp_splits)
        ...
        q1, q2 = ops.split(
            ...
            num_outputs=2,
            axis=-1,
            splits=[self.rotary_dim, self.rotary_dim],
            name="rope_split_q",
        )
        k1, k2 = ops.split(
            graph,
        ...
        layer_present = self.kv_cache(graph, kv, layer_past, self.sequence_length)
    ...
    def forward_attention(self, graph, q, k, attention_mask, softmax_type):
        ...
        layer_scale_coeff_value = np.array([self.layer_index + 1], dtype=np.float16)
        ...
        score = ops.matmul(graph, q, temp_k, **self.matmul_kwargs)
    ...
    def forward_output(self, graph, score, v):
        score = ops.matmul(graph, score, v, **self.matmul_kwargs)
        score = ops.transpose(graph, score, [0, 2, 1, 3])
        score = ops.reshape(
            graph, score, [self.batch_size, self.sequence_length, self.input_size]
        )
        output = self.c_proj(graph, score, **self.matmul_kwargs)
        return output
...
class RotaryAttention(TPRotaryAttention, ShardRotaryAttention):
    layer_class_map = {
        "tp": TPRotaryAttention,
        "shard": ShardRotaryAttention,
    }
